...
class AddressTests: XCTestCase {
    ...
    func testSplitRegion() {
        ...
        let region = PhysPageAlignedRegion(physAddr, pageSize: PageSize(), pageCount: 256)
        XCTAssertEqual(region.pageCount, 256)
        XCTAssertEqual(region.pageSize.size, 4096)
        XCTAssertEqual(region.baseAddress , physAddr)

        let (newRegion, result) = region.splitRegion(withFirstRegionCount: 255)
        XCTAssertEqual(newRegion.pageCount, 255)
        XCTAssertEqual(newRegion.pageSize.size, 4096)
        XCTAssertEqual(newRegion.baseAddress, physAddr)

        XCTAssertEqual(result.pageCount, 1)
        XCTAssertEqual(result.pageSize.size, 4096)
        XCTAssertEqual(result.baseAddress, newRegion.endAddress + 1)

        let (newRegion2, result2) = newRegion.splitRegion(withFirstRegionCount: 254)
        XCTAssertEqual(newRegion2.pageCount, 254)
        XCTAssertEqual(newRegion2.pageSize.size, 4096)
        XCTAssertEqual(newRegion2.baseAddress, physAddr)

        XCTAssertEqual(result2.pageCount, 1)
        XCTAssertEqual(result2.pageSize.size, 4096)
        XCTAssertEqual(result2.baseAddress, newRegion2.endAddress + 1)
    }


    func testPhysPageAlignedRegionIterator() {
        let pageRange = PhysPageAlignedRegion(PhysAddress(4096), pageSize: PageSize(), pageCount: 4)
        XCTAssertEqual(pageRange.baseAddress, PhysAddress(4096))
    ...
    func testCreateRanges() {
        do {
            ...
            XCTAssertEqual(physPageRanges[0].baseAddress, start)
        ...
        do {
            ...
            XCTAssertEqual(physPageRanges[0].pageSize.size, 0x1000)
            XCTAssertEqual(physPageRanges[0].baseAddress, PhysAddress(start))
            ...
            XCTAssertEqual(physPageRanges[1].baseAddress, PhysAddress(start) + 0x3000)
            ...
            XCTAssertEqual(physPageRanges[2].baseAddress, PhysAddress(start) + 0x3000 +  0x400_000)
            XCTAssertEqual(physPageRanges[2].endAddress, PhysAddress(end) - 0x2001)
            XCTAssertEqual(physPageRanges[2].pageCount, 1)

            XCTAssertEqual(physPageRanges[3].pageSize.size, 0x1000)
            XCTAssertEqual(physPageRanges[3].baseAddress, PhysAddress(end) - 0x2000)
        ...
        do {
            ...
            XCTAssertEqual(physPageRanges[0].pageSize.size, 0x1000)
            XCTAssertEqual(physPageRanges[0].baseAddress, PhysAddress(0x1000))
            XCTAssertEqual(physPageRanges[0].endAddress, PhysAddress(0x1ff_fff)) // 2MB - 1
            XCTAssertEqual(physPageRanges[0].pageCount, 511)

            XCTAssertEqual(physPageRanges[1].pageSize.size, 0x200_000)
            XCTAssertEqual(physPageRanges[1].baseAddress, PhysAddress(0x200_000))
        ...
        do {
            ...
            let ranges = PhysPageAlignedRegion.createRanges(startAddress: PhysAddress(0), endAddress: PhysAddress(2 * gb - 1), pageSizes: [PageSize()])
            XCTAssertEqual(ranges.count, 1)
            let range = ranges[0]
            XCTAssertEqual(range.pageCount, 524288)
            XCTAssertEqual(range.pageSize.size, 4096)
            XCTAssertEqual(range.baseAddress, PhysAddress(0))
            let end = 2 * gb - 1
        ...
        do {
            ...
            let ranges = PhysPageAlignedRegion.createRanges(startAddress: PhysAddress(0), endAddress: PhysAddress(99), pageSizes: [PageSize()])
            XCTAssertEqual(ranges.count, 0)
        }

        do {
            // Region overlaps 2 pages but doesnt cover either page fully
            let ranges = PhysPageAlignedRegion.createRanges(startAddress: PhysAddress(1), endAddress: PhysAddress(6000), pageSizes: [PageSize()])
            XCTAssertEqual(ranges.count, 0)
        }

        do {
            // Region overlaps 3 pages but only covers 1 fully
            let ranges = PhysPageAlignedRegion.createRanges(startAddress: PhysAddress(100), endAddress: PhysAddress(9099), pageSizes: [PageSize()])
            XCTAssertEqual(ranges.count, 1)
            XCTAssertEqual(ranges[0].pageCount, 1)
            XCTAssertEqual(ranges[0].pageSize.size, 4096)
            XCTAssertEqual(ranges[0].baseAddress, PhysAddress(4096))
        }

        do {
            // 1Gb pages only
            let ranges = PhysPageAlignedRegion.createRanges(startAddress: PhysAddress(0), endAddress: PhysAddress(1 * gb - 1), pageSizes: [PageSize(1 * gb)])
            XCTAssertEqual(ranges.count, 1)
            XCTAssertEqual(ranges[0].pageCount, 1)
            XCTAssertEqual(ranges[0].pageSize.size, 1 * gb)
            XCTAssertEqual(ranges[0].baseAddress, PhysAddress(0))
        }

        do {
            // 1Gb pages only
            let ranges = PhysPageAlignedRegion.createRanges(startAddress: PhysAddress(0), endAddress: PhysAddress(1 * gb), pageSizes: [PageSize(1 * gb)])
            XCTAssertEqual(ranges.count, 1)
            XCTAssertEqual(ranges[0].pageCount, 1)
            XCTAssertEqual(ranges[0].pageSize.size, 1 * gb)
            XCTAssertEqual(ranges[0].baseAddress, PhysAddress(0))
        ...
        do {
            // 1Gb pages only
            let ranges = PhysPageAlignedRegion.createRanges(startAddress: PhysAddress(0), endAddress: PhysAddress(1 * gb + 4095), pageSizes: [PageSize(1 * gb)])
            XCTAssertEqual(ranges.count, 1)
            XCTAssertEqual(ranges[0].pageCount, 1)
            XCTAssertEqual(ranges[0].pageSize.size, 1 * gb)
            XCTAssertEqual(ranges[0].baseAddress, PhysAddress(0))
    ...
    func testPhysPageAlignedRegion() {
        let range = PhysPageAlignedRegion(start: PhysAddress(0xE0000), size: 0x20000)
        XCTAssertEqual(range.baseAddress, PhysAddress(0xE0000))
